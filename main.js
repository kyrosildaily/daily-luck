...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            if (!user.emailVerified) {
                showView('verify');
                document.getElementById('verif-email').textContent = user.email;
    ...
    signupButton.addEventListener('click', () => {
        ...
        auth.createUserWithEmailAndPassword(email, password)
            .then(userCredential => {
                ...
                return db.collection('users').doc(user.uid).set({ name, surname, email, socialProfiles, createdAt: firebase.firestore.FieldValue.serverTimestamp(), profileComplete: true, lastClaim: null });
            })
    ...
    googleLoginButton.addEventListener('click', () => {
        ...
        auth.signInWithPopup(provider)
            .then(async (result) => {
                ...
                if (!doc.exists) {
                    const newUserData = {
                        name: user.displayName.split(' ')[0], surname: user.displayName.split(' ').slice(1).join(' '), email: user.email, createdAt: firebase.firestore.FieldValue.serverTimestamp(), profileComplete: false, socialProfiles: {}
                    };
    ...
    claimButton.addEventListener('click', async () => {
        ...
        if (doc.exists && doc.data().lastClaim) {
            ...
            if (now.getTime() - lastClaimDate.getTime() < (24 * 60 * 60 * 1000)) {
                claimButton.textContent = translations[currentLang].comeBackTomorrow;
                return alert(translations[currentLang].limitUsed);
            }
        }
        ...
        setTimeout(async () => {
            ...
            await userRef.update({ lastClaim: now });
            claimButton.textContent = translations[currentLang].comeBackTomorrow;
