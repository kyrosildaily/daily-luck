...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if (!user.emailVerified) {
            ...
            if(spinCountEl) spinCountEl.textContent = userData.dailySpins || 0;
...
claimButton.addEventListener('click', async () => {
    ...
    setTimeout(async () => {
        ...
        if(winner.id === 'try-again') {
            message = winnerName; document.getElementById('message-area').style.backgroundColor = '#ffc107';
        } else {
