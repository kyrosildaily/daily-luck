...
const displayWinnings = async (userId) => {
    ...
    if (querySnapshot.empty) {
        ...
        querySnapshot.forEach(doc => {
            ...
            winningsListEl.innerHTML += `<li><span>${data.rewardName}</span><span class="date">${formattedDate}</span></li>`;
        });
    }
};
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if (!doc.exists) return; // Should not happen with new users, but as a safeguard.
        ...
        if (now.setHours(0,0,0,0) > lastLogin.setHours(0,0,0,0)) {
            userData.dailySpins = 1; // Give one free spin for the new day
            await userRef.update({ dailySpins: 1, lastLogin: now });
        }
        
        // Route user based on profile completion and email verification
