...
        onboardingSubmitButton: document.getElementById('onboarding-submit'),
        editProfileButton: document.getElementById('edit-profile-button'),
        editProfileSubmitButton: document.getElementById('edit-profile-submit'),
        editProfileCancelButton: document.getElementById('edit-profile-cancel'),
...
        ui.langEnButton.classList.toggle('active', lang === 'en');
        ui.langTrButton.classList.toggle('active', lang === 'tr');
    };
...
            ui.claimButton.textContent = translations[currentLang].comeBackTomorrow;
        } else {
            ui.claimButton.disabled = false;
            ui.claimButton.textContent = translations[currentLang].tryChanceButton;
        }
...
    ui.showSignup.addEventListener('click', (e) => { e.preventDefault(); ui.loginForm.classList.add('hidden'); ui.signupForm.classList.remove('hidden'); });
    ui.showLogin.addEventListener('click', (e) => { e.preventDefault(); ui.signupForm.classList.add('hidden'); ui.loginForm.classList.remove('hidden'); });

    ui.onboardingSubmitButton.addEventListener('click', async () => {
...
    });
    
    ui.editProfileButton.addEventListener('click', async () => {
        const user = auth.currentUser; if (!user) return;
...
    ui.editProfileCancelButton.addEventListener('click', () => showView('app'));

    ui.editProfileSubmitButton.addEventListener('click', async () => {
        const user = auth.currentUser; if (!user) return;
        const userRef = db.collection('users').doc(user.uid);
...
                    setLanguage(currentLang);
                } else {
                    // Existing user, onAuthStateChanged will handle rendering correctly
                }
            })
...
        
        ui.claimButton.disabled = true; ui.claimButton.textContent = translations[currentLang].spinningButton;
